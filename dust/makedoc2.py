...
def build_nav_structure(docs_dir: Path, created_files: List[Path]) -> Dict[str, Union[str, List, Dict]]:
...
def dict_to_mkdocs_nav(nav_dict: Dict, level: int = 0) -> List[Union[str, Dict]]:
    ...
    for key, value in nav_dict.items():
        if isinstance(value, str):
            ...
            if len(value) == 1 and 'Introduction' in value:
                # If there's only an Introduction, flatten it
                nav_list.append({key: value['Introduction']})
            else:
                # Create nested structure
                nested_nav = dict_to_mkdocs_nav(value, level + 1)
                nav_list.append({key: nested_nav})
...
def main():
    ...
    print(f"Traversing {src_dir} and creating documentation in {docs_api_dir}...")
    ...
    mkdocs_nav = {'nav': [{'API Reference': nav_list}]}
    
    # Output YAML
    print("\nGenerated MkDocs navigation YAML:")
    print("=" * 50)
    yaml_output = yaml.dump(mkdocs_nav, default_flow_style=False, sort_keys=False)
    print(yaml_output)
    
    # Optionally save to file
    nav_file = Path('mkdocs_nav.yml')

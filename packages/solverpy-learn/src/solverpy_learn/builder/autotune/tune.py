...
def leaves_grid(
   ...
   **args: Any,
   ...
   return tune(check.leaves, nick, sampler=sampler, **args)


def bagging(nick: str = "bagging", **args: Any) -> TuneResult:
   return tune(check.bagging, nick, **args)


def min_data(nick: str = "min_data", **args: Any) -> TuneResult:
   values = [5, 10, 20, 50, 100, 500, 1000, 2000, 5000, 10000]
   sampler = optuna.samplers.GridSampler({"min_data": values})
   return tune(check.min_data, nick, sampler=sampler, **args)


def regular(nick: str = "regular", **args: Any) -> TuneResult:
   return tune(check.regular, nick, **args)


def depth(nick: str = "depth", **args: Any) -> TuneResult:
   return tune(check.depth, nick, **args)


def learning_rate(nick: str = "learning_rate", **args: Any) -> TuneResult:
   values = [0.01, 0.05, 0.1, 0.15, 0.2, 0.25]
   sampler = optuna.samplers.GridSampler({"learning_rate": values})
   return tune(check.learning_rate, nick, sampler=sampler, **args)


def posneg_weight(nick: str = "posneg", **args: Any) -> TuneResult:
   ...
   return tune(check.posneg_weight,
               nick,
               sampler=sampler,
               posneg_base=posneg_base,
               **args)
